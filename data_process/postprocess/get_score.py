...
from util.list_util import deduplication_keep_order, longest_common_sublist
from util.task_util import bio2phrases
...
def remove_mask(line, mask_num, total_mask_num):
    ...
    line = line.copy()
    while loop < len(line):
        if line[loop] == "[MASK]" and find_sublist(line[:loop + 1], ["other", "phrases", "is"]) == -1:
...
def get_score(items, cached_path, mask_num, total_mask_num):
    contents, kws = item2token(items, cached_path=cached_path)
    path = "results"
    ...
    test_tagging_c_lines = read_lines(os.path.join(path, "test_tagging_c_results.txt"))
    test_tagging_c_prob_lines = read_lines(os.path.join(path, "test_tagging_c_results_prob.txt"))
    test_tagging_p_lines = read_lines(os.path.join(path, "test_tagging_p_results.txt"))
    test_tagging_p_prob_lines = read_lines(os.path.join(path, "test_tagging_p_results_prob.txt"))
    ...
    for idx, (tagging_line, tagging_prob_line) in enumerate(zip(test_tagging_p_lines, test_tagging_p_prob_lines)):
        ...
        tagging_line = tagging_line[1:]
        tagging_prob_line = tagging_prob_line[1:]
...
def get_score2(items, cached_path, mask_num, total_mask_num):
    contents, kws = item2token(items, cached_path=cached_path)
    path = "results"
    seq_in_lines = read_lines(os.path.join(path, "seq.in"))
    test_result_lines = read_lines(os.path.join(path, "test_results.txt"))
