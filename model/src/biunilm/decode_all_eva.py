...
def main():
    s2l_time, avg_present, avg_center, seq_in_lines, present_all = seq2label()
    s2s_time, avg_absent, absent_all = seq2seq(seq_in_lines)
...
def seq2label():
    ...
    for model_recover_path in glob.glob(args.model_recover_path.strip()):
        ...
        new_seq_in_lines = tagging2test(
            ...
            has_prompt=False if args.has_prompt == "False" else True
        )

        print("***** ABSENT *****")
        if args.absent_type == "GC":
            new_seq_in_lines = in2test(read_lines(os.path.join(args.result_dir, "seq.in")), args.mask_num)
        return time_end - time_start, avg_present, avg_center, new_seq_in_lines, present_all
...
def seq2seq(input_lines):
    ...
    for model_recover_path in glob.glob(args.model_recover_path.strip()):
        ...
        return time_end - time_start, avg_absent, absent_all
