...
class DataPrefetcher():
    ...
    def preload(self):
        ...
        with torch.cuda.stream(self.stream):
            self.batch = [
                t.to(self.device, non_blocking=True) if t is not None else None for t in self.batch]
...
def main():
    ...
    if args.do_train:
        ...
        bi_uni_pipeline = [Preprocess4Seq2seq(args.max_pred, args.mask_prob,
                                              ...
                                              mode="s2s", has_oracle=args.has_sentence_oracle,
        ...
        fn_labelp = os.path.join(
            args.data_dir, args.labelp_file if args.labelp_file else 'train.labelp')
        fn_labelc = os.path.join(
            args.data_dir, args.labelc_file if args.labelc_file else 'train.labelp')
        train_dataset = Seq2SeqDataset(fn_src, fn_tgt, fn_labelp, fn_labelc, args.train_batch_size, \
        ...
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=_batch_size, sampler=train_sampler,
                                                       num_workers=args.num_workers, collate_fn=batch_list_to_batch_tensors, pin_memory=True)
    ...
    if (recover_step is None) and (args.model_recover_path is None):
        ...
        model = BertForPreTrainingLossMask.from_pretrained(args.bert_model, state_dict=_state_dict, num_seq_labels=len(label_list),
                                                           num_labels=cls_num_labels, num_rel=0,
                                                           type_vocab_size=type_vocab_size, config_path=args.config_path,
                                                           task_idx=3, num_sentlvl_labels=num_sentlvl_labels,
        ...
        if recover_step:
            ...
            global_step = 0
        model = BertForPreTrainingLossMask.from_pretrained(args.bert_model, state_dict=model_recover, num_seq_labels=len(label_list),
                                                           num_labels=cls_num_labels, num_rel=0,
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs)+1, desc="Epoch", disable=args.local_rank not in (-1, 0)):
            ...
            #    batch = [
            #        t.to(device, non_blocking=True) if t is not None else None for t in batch]
            ...
            while batch is not None:
                ...
                input_ids, segment_ids, input_mask, \
                    labelp_ids, labelc_ids, labelp_mask, labelc_mask, \
                    mask_qkv, lm_label_ids, masked_pos, masked_weights, is_next, task_idx = batch
                oracle_pos, oracle_weights, oracle_labels = None, None, None

                loss_tuple = model(input_ids, segment_ids, input_mask, \
                                   labelp_ids, labelc_ids, labelp_mask, labelc_mask, \
                                   ...
                                   masked_labels_2=oracle_labels, mask_qkv=mask_qkv, num_seq_labels=len(label_list))
                masked_lm_loss, seq_labelp_loss, seq_labelc_loss, next_sentence_loss = loss_tuple
                if n_gpu > 1:    # mean() to average on multi-gpu.
                    # loss = loss.mean()
                    masked_lm_loss = masked_lm_loss.mean()
                    next_sentence_loss = next_sentence_loss.mean()
                    seq_labelp_loss = seq_labelp_loss.mean()
                    seq_labelc_loss = seq_labelc_loss.mean()
                loss = masked_lm_loss + seq_labelp_loss + seq_labelc_loss
                #loss = seq_labelp_loss
                
                # logging for each step (i.e., before normalization by args.gradient_accumulation_steps)
                # iter_bar.set_description('Iter (loss=%5.3f)' % loss.item())
                # iter_bar.set_description('Iter (mlm_loss=%5.3f | labelp=%5.3f | labelc=%5.3f | total_loss=%5.3f)' % \
                #     (masked_lm_loss.item(), seq_labelp_loss.item(), seq_labelc_loss.item(), loss.item()))
                if iter_id % 5 == 0:
                    logger.info('Iter %d (mlm_loss=%5.3f | labelp=%5.3f | labelc=%5.3f | total_loss=%5.3f)' % \
                        (iter_id, masked_lm_loss.item(), seq_labelp_loss.item(), seq_labelc_loss.item(), loss.item()))
                    #logger.info('Iter %d (mlm_loss=%5.3f | labelp=%5.3f | labelc=%5.3f | total_loss=%5.3f)' % \
                    #    (iter_id, 0.0, seq_labelp_loss.item(), 0.0, loss.item()))
                ...
                if args.fp16:
                    ...
                    loss.backward()
                if (iter_id + 1) % args.gradient_accumulation_steps == 0:
