...
def all_correct(prediction, kws, top_n):
    res = []
    assert len(prediction) == len(kws)
    prediction = [[pp[0] for pp in sorted(p, key=lambda t: t[1], reverse=True)] for p in prediction]
    prediction = [deduplication_keep_order(p)[0:min(top_n, len(deduplication_keep_order(p)))] for p in prediction]
    ...
    for i in tqdm(range(len(prediction))):
        ...
        for pre in prediction_line:
            if pre not in kw:
                is_ac = False
                break
        res.append(prediction_line if is_ac else ["FALSE!!!"])
    return res
...
def get_score(path, test_tagging_c_lines,
              test_tagging_c_prob_lines, test_tagging_p_lines, test_tagging_p_prob_lines, mask_num=3, total_mask_num=3):
    ...
    print(f"{str(p_5_p)[:5]}  {str(r_5_p)[:5]}  {str(f1_score(p_5_p, r_5_p))[:5]} \t"
          ...
          # print("********** all **********")
          f"{str(p_5_all)[:5]}  {str(r_5_all)[:5]}  {str(f1_score(p_5_all, r_5_all))[:5]}\t"
          f"{str(p_all_all)[:5]}  {str(r_all_all)[:5]}  {str(f1_score(p_all_all, r_all_all))[:5]}\t")


def get_score_present(path, test_tagging_p_lines, test_tagging_p_prob_lines, mask_num=3, total_mask_num=3):
    bert_model = "/root_path/PTModel/unilm_pytorch/bert-base-cased"
    tokenizer = BertTokenizer.from_pretrained(bert_model, do_lower_case=True)
    items = load_dataset(os.path.join(path, "data.json"))
    contents, kws = item2token(items, tokenizer, cached_path=os.path.join(path, "cache"))
...
def get_score_absent(path, test_result_lines, test_result_prob_lines, absent_type, mask_num=3, total_mask_num=3):
    ...
    for i in tqdm(range(len(seq_in_lines))):
        ...
        seq_in_line = seq_in_lines[i].split(" ")
        seq_in_line = remove_mask(seq_in_line, mask_num, total_mask_num)
        if "other" in seq_in_line[0:seq_in_line.index("[SEP]")]:
            max_id = seq_in_line.index("other")
        else:
            max_id = find_sublist(seq_in_line, ["[MASK]"] * 8)
        assert max_id >= 0
        start_center = False
        has_words = False
        test_result_line = test_result_lines[i].split(" ")
    ...
    ac_all = all_correct(prediction_absent_all, kws_absent_all, top_n=10000000)
    write_lines(os.path.join(path, "output", "all_correct"),
                [" \\\\ ".join([pp for pp in p]) for p in ac_all], "w+")
    ...
    print(f"Absent DupRatio = {1 - sum([len(set([tuple(xx) for xx in x])) for x in absent_stem])/sum([len(x) for x in absent_stem])}")
    return sum([len(set([tuple(pp[0]) for pp in p]))
